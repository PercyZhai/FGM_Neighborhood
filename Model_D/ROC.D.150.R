...
working.path <- "/home/percysjzhai/GroupLasso/Model_D"
save.path <- "/home/percysjzhai/GroupLasso/Model_D/Results/p150/ROC"

# For local testing, use the following paths:
#func.path <- "~/GitHub/GroupLasso/Functions"
#working.path <- "~/GitHub/GroupLasso/Model_D"
#save.path <- "~/GitHub/GroupLasso/Model_D/Results/p150/ROC"
...
source(paste(working.path,"M3.prelim.func.R", sep="/"))
...
set.seed(run.ind)
Theta <- cov.mat.model3(p, mu) # p*mu by p*mu large square matrix
G.true.3 <- matrix(0, p, p) # p by p adjacency matrix
for(i in 1:p){
  for(j in 1:p){
...
# For the use of PSKL basis group - exclusively for model D

fpc.score.pskl <- numeric(0)
for(j in 1:p){
  obs.val.matrix <- matrix(0, nrow=tau, ncol=n)
  for (i in c(1:n)){
    obs.val.vec <- as.vector(h[i, j, ])
  ...
  bspline.basis <- create.bspline.basis(rangeval=c(0, 1), nbasis=M)
  b.fou <- obs.fourier.bases(obs.time, M)
  fpc.score.pskl <- matrix(NA, n, p*M)
  for(i in 1:n){
    for(j in 1:p){
      fpc.score.pskl[i, (j-1)*M + (1:M)] <- t(b.fou) %*% h[i,j,] / tau
    }
  }
}
...
for(k in 1:n.a){
  ...
  TFPR.zapata.k <- foreach(l = 1:L, .combine="rbind", .packages="fgm") %dopar% {
    ...
    TPR.zapata.l <- prec.rec(G.true.3, G.zapata, type="AND")$TPR
    FPR.zapata.l <- prec.rec(G.true.3, G.zapata, type="AND")$FPR
...
TFPR.qiao <- foreach(l = 1:L, .combine="rbind", .packages="matrixcalc") %dopar% {
  ...
  TPR.qiao.l <- prec.rec(G.true.3, G.qiao, type="AND")$TPR
  FPR.qiao.l <- prec.rec(G.true.3, G.qiao, type="AND")$FPR
...
for(l in 1:L){
  TPR.and[l] <- prec.rec(G.true.3, G.list[[l]], type="AND")$TPR
  FPR.and[l] <- prec.rec(G.true.3, G.list[[l]], type="AND")$FPR
  TPR.or[l] <- prec.rec(G.true.3, G.list[[l]], type="OR")$TPR
  FPR.or[l] <- prec.rec(G.true.3, G.list[[l]], type="OR")$FPR
}
...
# Calculate TPR and FPR
for(l in 1:L){
  TPR.FPC.and[l] <- prec.rec(G.true.3, G.list[[l]], type="AND")$TPR
  FPR.FPC.and[l] <- prec.rec(G.true.3, G.list[[l]], type="AND")$FPR
  TPR.FPC.or[l] <- prec.rec(G.true.3, G.list[[l]], type="OR")$TPR
  FPR.FPC.or[l] <- prec.rec(G.true.3, G.list[[l]], type="OR")$FPR
}

# Finally, for Model D, PSKL basis
####################################

G.list <- list()
TPR.PSKL.and <- rep(0,L)
FPR.PSKL.and <- rep(0,L)
TPR.PSKL.or <- rep(0,L)
FPR.PSKL.or <- rep(0,L)
...
V.array <- foreach(j = 1:p, .combine="rbind") %dopar% {
  jth.range <- (((j-1)*M+1) : (j*M))
  for(i in 1:n){
    Y[i,] <- fpc.score.pskl[i, jth.range]
    X[i,] <- fpc.score.pskl[i, -jth.range]
  }
...
# Calculate TPR and FPR
for(l in 1:L){
  TPR.PSKL.and[l] <- prec.rec(G.true.3, G.list[[l]], type="AND")$TPR
  FPR.PSKL.and[l] <- prec.rec(G.true.3, G.list[[l]], type="AND")$FPR
  TPR.PSKL.or[l] <- prec.rec(G.true.3, G.list[[l]], type="OR")$TPR
  FPR.PSKL.or[l] <- prec.rec(G.true.3, G.list[[l]], type="OR")$FPR
}
...
roc <- cbind(TPR.and, FPR.and, TPR.or, FPR.or, TPR.qiao, FPR.qiao, TPR.zapata, FPR.zapata, TPR.FPC.and, 
             FPR.FPC.and, TPR.FPC.or, FPR.FPC.or, TPR.PSKL.and, FPR.PSKL.and, TPR.PSKL.or, FPR.PSKL.or)
...
save(roc, file=paste(save.path,"/ROC.D.150.RunInd",run.ind,".Rdata",sep=""))
