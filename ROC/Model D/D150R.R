...
# 3.2.3. maximum lambda for ADMM with PSKL basis
lambda.max.PSKL <- lambda.sup.global.gX(fpc.score.pskl, p)
lambdas.PSKL <- seq(lambda.max.PSKL, 0, length.out=L)
...
################################################
##                                            ##
##  PART 7. ADMM-PSKL OVER EACH j AND lambda  ##
##                                            ##
################################################
time.start.PSKL <- proc.time()
...
V.array <- foreach(j = 1:p, .combine="rbind") %dopar% {
  jth.range <- (((j-1)*M+1) : (j*M))
  for(i in 1:n){
    A.Y[i,] <- fpc.score.pskl[i, jth.range]
    A.X[i,] <- fpc.score.pskl[i, -jth.range]
  }
...
# Calculate TPR and FPR
for(l in 1:L){
  TPR.PSKL.and[l] <- prec.rec(G.true, G.list[[l]], type="AND")$TPR
  FPR.PSKL.and[l] <- prec.rec(G.true, G.list[[l]], type="AND")$FPR
  TPR.PSKL.or[l] <- prec.rec(G.true, G.list[[l]], type="OR")$TPR
  FPR.PSKL.or[l] <- prec.rec(G.true, G.list[[l]], type="OR")$FPR
}
...
roc <- cbind(TPR.and.gX, FPR.and.gX, TPR.or.gX, FPR.or.gX, TPR.fglasso, FPR.fglasso,
             ...
             TPR.PSKL.and, FPR.PSKL.and, TPR.PSKL.or, FPR.PSKL.or)
...
#      PART 8: SAVE RUNTIME        #
####################################
roc.runtime <- c(runtime.gX, runtime.fglasso, runtime.pskl, runtime.gY, runtime.fpc, runtime.PSKL)
save(roc.runtime, file=paste(runtime.path,
                             "/ROCtime.D.150.Runind",run.ind,".Rdata",sep=""))
