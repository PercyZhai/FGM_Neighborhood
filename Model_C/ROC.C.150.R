...
working.path <- "/home/percysjzhai/GroupLasso/Model_C"
save.path <- "/home/percysjzhai/GroupLasso/Model_C/Results/p150/ROC"

# For local testing, use the following paths:
#func.path <- "~/GitHub/GroupLasso/Functions"
#working.path <- "~/GitHub/GroupLasso/Model_C"
#save.path <- "~/GitHub/GroupLasso/Model_C/Results/p150/ROC"
...
source(paste(working.path,"M5.prelim.func.R", sep="/"))
...
G.list <- edge.sets(G.true.5, M=mu, seed=seed)
Omega.list <- Omega.gen(G.list, seed=seed)
Sigma.5 <- Sigma.mod.5(Omega.list)

tr.Sigma <- rep(0,mu)
for(l in 1:mu){
  Sigma.l <- Sigma.5[(l-1)*p + 1:p, (l-1)*p + 1:p]
  tr.Sigma[l] <- sum(diag(Sigma.l))
}


# 1. Generating theta
theta <- rmvnorm(n, sigma = Sigma.5)
...
for(i in 1:n){
  for(j in 1:p){
    theta.ij <- matrix(NA, mu, 1)
    for(l in 1:mu){
      theta.ij[l,1] <- theta[i, (l-1)*p+j]
    }
    h[i,j,] <- b.mat %*% theta.ij + matrix(rnorm(tau, 0, sqrt(0.05*(sum(tr.Sigma))/p)), ncol=1)
  }
}
...
for(k in 1:n.a){
  ...
  TFPR.zapata.k <- foreach(l = 1:L, .combine="rbind", .packages="fgm") %dopar% {
    ...
    TPR.zapata.l <- prec.rec(G.true.5, G.zapata, type="AND")$TPR
    FPR.zapata.l <- prec.rec(G.true.5, G.zapata, type="AND")$FPR
...
TFPR.qiao <- foreach(l = 1:L, .combine="rbind", .packages="matrixcalc") %dopar% {
  ...
  TPR.qiao.l <- prec.rec(G.true.5, G.qiao, type="AND")$TPR
  FPR.qiao.l <- prec.rec(G.true.5, G.qiao, type="AND")$FPR
...
for(l in 1:L){
  TPR.and[l] <- prec.rec(G.true.5, G.list[[l]], type="AND")$TPR
  FPR.and[l] <- prec.rec(G.true.5, G.list[[l]], type="AND")$FPR
  TPR.or[l] <- prec.rec(G.true.5, G.list[[l]], type="OR")$TPR
  FPR.or[l] <- prec.rec(G.true.5, G.list[[l]], type="OR")$FPR
}
...
# Calculate TPR and FPR
for(l in 1:L){
  TPR.FPC.and[l] <- prec.rec(G.true.5, G.list[[l]], type="AND")$TPR
  FPR.FPC.and[l] <- prec.rec(G.true.5, G.list[[l]], type="AND")$FPR
  TPR.FPC.or[l] <- prec.rec(G.true.5, G.list[[l]], type="OR")$TPR
  FPR.FPC.or[l] <- prec.rec(G.true.5, G.list[[l]], type="OR")$FPR
}
...
save(roc, file=paste(save.path,"/ROC.C.150.RunInd",run.ind,".Rdata",sep=""))
