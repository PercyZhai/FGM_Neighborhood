...
G.mat.autism <- foreach(j = 1:p, .combine="rbind") %dopar% {
...
G.mat.control <- foreach(j = 1:p, .combine="rbind") %dopar% {
...
####################################
save(G.mat.autism,    file = paste(save.path, "G.mat.autism.RData",    sep="/"))
save(G.mat.control, file = paste(save.path, "G.mat.control.RData", sep="/"))
