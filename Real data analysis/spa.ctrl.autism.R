...
load(paste(data.path, "time.series.ABIDE.RData", sep="/"))
n.1 <- dim(h.1)[1] #autism group
...
################################################
##       PART 4.1. G.mat for autism Group     ##
################################################

B.mat.autism <- foreach(j = 1:p, .combine="rbind") %dopar% {
...
B.autism.sym <- matrix(NA, p, p)
for(j in 1:p){
  for(k in 1:p){
    B.autism.sym[j,k] <- min(B.mat.autism[j,k], B.mat.autism[k,j]) # equivalent to AND
  }
}

thres.vec <- seq(0, max(B.autism.sym)*1.01, length.out=500)

for(i in 1:500){
  threshold <- thres.vec[i]
  G.autism.sym <- matrix(0, p, p)
  for(j in 1:p){
    for(k in 1:p){
      G.autism.sym[j,k] <- as.numeric(B.autism.sym[j,k] > threshold)
      if(j==k) G.autism.sym[j,k] <- 0
    }
  }
  if(sum(G.autism.sym) <= spar.ctrl * p*(p-1)) break
}

time.end.autism <- proc.time()
runtime.autism <- (time.end.autism - time.start.autism)[3]
...
####################################
save(G.autism.sym,    file = paste(save.path, "G.spa.ctrl.autism.RData",    sep="/"))
...
#      PART 6: SAVE RUNTIME        #
####################################
# real.data.runtime <- c(runtime.autism, runtime.control, runtime.fpc)
