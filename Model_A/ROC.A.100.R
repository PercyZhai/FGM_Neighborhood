...
working.path <- "/home/percysjzhai/GroupLasso/Model_A"
save.path <- "/home/percysjzhai/GroupLasso/Model_A/Results/p100/ROC"

# For local testing, use the following paths:
#func.path <- "~/GitHub/GroupLasso/Functions"
#working.path <- "~/GitHub/GroupLasso/Model_A"
#save.path <- "~/GitHub/GroupLasso/Model_A/Results/p100/ROC"
...
source(paste(working.path,"M6.prelim.func.R", sep="/"))
...
set.seed(run.ind)
Theta <- cov.mat.model6(p, mu) # p*mu by p*mu large square matrix
G.true.6 <- matrix(0, p, p) # p by p adjacency matrix
for(i in 1:p){
  for(j in 1:p){
...
for(l in 1:L){
  TPR.FPC.and[l] <- prec.rec(G.true.6, G.list[[l]], type="AND")$TPR
  FPR.FPC.and[l] <- prec.rec(G.true.6, G.list[[l]], type="AND")$FPR
  TPR.FPC.or[l] <- prec.rec(G.true.6, G.list[[l]], type="OR")$TPR
  FPR.FPC.or[l] <- prec.rec(G.true.6, G.list[[l]], type="OR")$FPR
}
...
TFPR.qiao <- foreach(l = 1:L, .combine="rbind", .packages="matrixcalc") %dopar% {
  ...
  TPR.qiao.l <- prec.rec(G.true.6, G.qiao, type="AND")$TPR
  FPR.qiao.l <- prec.rec(G.true.6, G.qiao, type="AND")$FPR
...
for(k in 1:n.a){
  ...
  TFPR.zapata.k <- foreach(l = 1:L, .combine="rbind", .packages="fgm") %dopar% {
    ...
    TPR.zapata.l <- prec.rec(G.true.6, G.zapata, type="AND")$TPR
    FPR.zapata.l <- prec.rec(G.true.6, G.zapata, type="AND")$FPR
...
# Calculate TPR and FPR
for(l in 1:L){
  TPR.and[l] <- prec.rec(G.true.6, G.list[[l]], type="AND")$TPR
  FPR.and[l] <- prec.rec(G.true.6, G.list[[l]], type="AND")$FPR
  TPR.or[l] <- prec.rec(G.true.6, G.list[[l]], type="OR")$TPR
  FPR.or[l] <- prec.rec(G.true.6, G.list[[l]], type="OR")$FPR
}
...
save(roc, file=paste(save.path,"/ROC.A.100.RunInd",run.ind,".Rdata",sep=""))
